...
object ConnHelper extends Serializable {
  lazy val jedis = new Jedis("192.168.74.128")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://192.168.74.128:27017/recommender"))
}
...
object OnlineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ssc = new StreamingContext(sc, Seconds(2)) // 批处理时间
      ...
      rdds =>
        rdds.foreach {
          case (uid, mid, score, timestamp) => {
            ...
            val OnlineRecs = computeMovieScores(candidateMovies, userRecentlyRatings, simMovieMatrixBroadCast.value)
  ...
  def getTopSimMovies(num: Int, mid: Int, uid: Int, simMovies: scala.collection.Map[Int, scala.collection.immutable.Map[Int, Double]])
    ...
    // 3. 把看过的过滤，得到输出列表 _1:mid
    allSimMovies.filter(x => !ratingExist.contains(x._1))
      .sortWith(_._2 > _._2) // 根据评分降序排序
      .take(num)
      .map(x => x._1)
  }
  ...
  def saveDataToMongoDB(uid: Int, OnlineRecs: Array[(Int, Double)])(implicit mongoConfig: MongoConfig): Unit = {
    ...
    // 将OnlineRecs数据存入表中
    OnlineRecsCollection.insert(MongoDBObject("uid" -> uid,
